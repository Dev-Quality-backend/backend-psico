...
app.use(cors({
  origin: ['http://localhost:3000', 'https://weak-lamb-shift.cyclic.app', 'https://psico-painel.vercel.app'],
...
app.post('/webhook/zoho', async (req, res) => {
  ...
  try {
    ...
    const [clientes] = await pool.execute('SELECT instituicaoNome FROM cadastro_clientes WHERE cpf = ?', [cpf]);
